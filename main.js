    ...
    async function runnabale() {
        ...
        try {
            const response = await axios.get(
                "https://api.spotify.com/v1/playlists/3WeU50f5AqaM0Z0aYQNoEz",
                {
                    headers: { Authorization: "Bearer " + OAuthSp },
                }
            );
            ...
            if (t >= timeSong) {
                const responsee = await axios.delete(
                    "https://api.spotify.com/v1/playlists/3WeU50f5AqaM0Z0aYQNoEz/tracks",
                    {
                        headers: {
                            Authorization: "Bearer " + OAuthSp,
                        },
                        data: {
                            tracks: [
                                {
                                    uri: response.data.tracks.items[0].track
                                        .uri,
                                    positions: [0],
                                },
                            ],
...
async function addToPlaylist(title, artist) {
    ...
    try {
        let res = await axios.get(
            `https://api.spotify.com/v1/search?q=${title.replace(
                " ",
                "+"
            )}+${artist.replace(" ", "+")}&type=track&limit=1`,
            { headers: { Authorization: "Bearer " + OAuthSp } }
        );
